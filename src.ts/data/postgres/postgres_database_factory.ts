...
export class PostgresDatabaseFactory extends DatabaseFactory implements FInitable {
	...
	public using<TResult>(executionContext: FExecutionContext, worker: (db: Database) => Promise<TResult>): Promise<TResult> {
		return FUsing(executionContext, this.create.bind(this), async (db: Database) => {
			try {
	...
	protected async onInit(): Promise<void> {
		...
		const maskedConnectionString: string = this._sqlConnectionUrl.toString();//MaskService.DEFAULT.maskUri(this._sqlConnectionUrl).toString();
